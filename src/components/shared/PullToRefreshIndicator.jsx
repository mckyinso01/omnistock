...
export default function PullToRefreshIndicator({ pulling, pullDistance, refreshing, threshold }) {
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-2 text-sm text-slate-500 transition-all ${ready ? "text-emerald-600" : ""}`}>
        {refreshing ? (
          <Loader2 className="w-5 h-5 animate-spin text-emerald-500" />
        ) : (
          <ArrowDown
            className={`w-5 h-5 transition-transform duration-200 ${ready ? "rotate-180 text-emerald-600" : ""}`}
